...
class Model implements Iterable<Model.Sq> {
    ...
    Model(int[][] solution) {
        ...
        this._board = new Sq[_width][_height];
        this._solnNumToPlace = new Place[_width * _height + 1];
        for (int col = 0; col < _width; col += 1) {
            for (int row = 0; row < _height; row += 1) {
                if (solution[col][row] != 0) {
                    this._solnNumToPlace[solution[col][row]]
                            = Place.pl(col, row);
                } else {
                    throw badArgs("Not all numbers from 1 - last appear.");
                }
            }
        }
        ...
        for (int col = 0; col < _width; col += 1) {
            for  (int row = 0; row < _height; row += 1) {
                if (solution[col][row] == 1 || solution[col][row] == last) {
                    this._board[col][row] = new Sq(col, row, solution[col][row],
                            true, this.arrowDirection(col, row), 0);
                } else {
                    this._board[col][row] = new Sq(col, row, 0,
                            false, this.arrowDirection(col, row), -1);
                }
            }
        }

        for (Sq[] col: this._board) {
            for (Sq square: col) {
                this._allSquares.add(square);
            }
        }

        for (Sq[] col: this._board) {
            for (Sq square: col) {
                square._successors = allSuccessors(square.pl, square._dir);
                square._predecessors = new PlaceList();
                for (Sq potential: this._allSquares) {
                    if (potential._sequenceNum != last) {
                        if (potential._dir
                                == Place.dirOf(potential.x, potential.y,
                                        square.x, square.y)) {
                            square._predecessors.add(potential.pl);
                        }
                    }
                }
            }
    ...
    Model(Model model) {
        ...
        for (Sq[] col: this._board) {
            for (Sq square: col) {
                Sq successor = model._board[square.x][square.y].successor();
                Sq predecessor = model._board[square.x][square.y].predecessor();
                Sq head = model._board[square.x][square.y].head();

                square._successor = (successor == null)
                        ? null : this.get(successor.x, successor.y);
                square._predecessor = (predecessor == null)
                        ? null : this.get(predecessor.x, predecessor.y);
                square._head = this.get(head.x, head.y);
            }
        }
    }
    ...
    boolean autoconnect() {
        ...
        for (int i = 0; i < this._allSquares.size() - 1; i += 1) {
            Sq square1 = this._allSquares.get(i);
            if (square1._sequenceNum != 0) {
                for (int j = i + 1; j < this._allSquares.size(); j += 1) {
                    Sq square2 = this._allSquares.get(j);
                    if (square2._sequenceNum != 0) {
                        if (square1.connectable(square2)) {
                            square1.connect(square2);
                            anyConnection = true;
                        } else if (square2.connectable(square1)) {
    ...
    public int arrowDirection(int x, int y) {
        ...
        if (seq0 < _width * _height) {
            int x1 = _solnNumToPlace[seq0 + 1].x;
            int y1 = _solnNumToPlace[seq0 + 1].y;
            int direction = Place.dirOf(x, y, x1, y1);
            return direction;
        } else {
            return 0;
        }
    ...
    final class Sq {
        ...
        boolean connectable(Sq s1) {
            if (this._dir == 0) {
                ...
                return pl.dirOf(s1.pl) == this._dir
                        && s1._predecessor == null
                        && this._successor == null
                        && s1._sequenceNum != 1
                        && (this._group == -1 || this._group != s1.group());
            } else if (s1._sequenceNum != 0 && this._sequenceNum != 0) {
                return pl.dirOf(s1.pl) == this._dir
                        && s1._predecessor == null
                        && this._successor == null
        ...
        boolean connect(Sq s1) {
            ...
            if (this._sequenceNum != 0 && s1._sequenceNum == 0) {
                int newSeqNum = this._sequenceNum + 1;
                Sq nextSq = this._successor;
                while (nextSq != null) {
                    nextSq._sequenceNum = newSeqNum;
                    newSeqNum += 1;
                ...
                int newSeqNum = s1._sequenceNum - 1;
                Sq lastSq = s1._predecessor;
                int toBeReleased = this._group;
                while (lastSq != null) {
                    lastSq._sequenceNum = newSeqNum;
                    lastSq._group = sgroup;
                    newSeqNum -= 1;
                    lastSq = lastSq._predecessor;
                }
                releaseGroup(toBeReleased);
            } else if (this._sequenceNum == 0 && s1._sequenceNum == 0) {
                if (this.group() == 0) {
                    this._group = -1;
                }
            ...
            for (Sq nextSq = this._head._successor;
                 nextSq != null; nextSq = nextSq._successor) {
                nextSq._head = this._head;
                nextSq._group = this._head._group;
            }
            return true;
        }
        ...
        void disconnect() {
            ...
            if (_sequenceNum == 0) {
                if (this._predecessor == null && next._successor == null) {
                    ...
                    Sq nextSq = next._successor;
                    while (nextSq != null) {
                        nextSq._group = next._group;
                        nextSq = nextSq._successor;
                    }
                }
            } else {
                predecessorHelper();
                successorsHelper(next);
            }
        ...
        /** Check if this square and its predecessors
         *  have any fixed predecessor.
         *  If none of them has fixed predecessor,
         *  set all these squares' sequence numbers to 0
         *  and create a new group for them.
         *  If not, set this.group to -1.
         */
        void predecessorHelper() {
            boolean allUnfixedPrecessors = true;

            if (this._hasFixedNum) {
                allUnfixedPrecessors = false;
            } else {
                for (Sq lastSq = this._predecessor;
                     lastSq != null; lastSq = lastSq._predecessor) {
                    if (lastSq._hasFixedNum) {
                        allUnfixedPrecessors = false;
                        break;
                    }
                }
            ...
            if (allUnfixedPrecessors) {
                Sq lastSq = this._predecessor;
                this._sequenceNum = 0;
                if (lastSq != null) {
                    int newGroup = newGroup();
                    this._group = newGroup;
                    for (; lastSq != null; lastSq = lastSq._predecessor) {
                        lastSq._sequenceNum = 0;
                        lastSq._group = newGroup;
                    }
                } else {
                    this._group = -1;
                }
        ...
        /** Check if next (this._successor) square and its successors
         *  have any fixed successor.
         *  If none of them has fixed successor,
         *  set all these squares' sequence numbers to 0
         *  and create a new group for them.
         *  If not, set next.group to -1.
         *
         * @param next the original sq of this._successor.
         */
        void successorsHelper(Sq next) {
            boolean allUnfixedSuccessors = true;

            if (next._hasFixedNum) {
                allUnfixedSuccessors = false;
            } else {
                Sq nextSq = next;
                for (; nextSq != null; nextSq = nextSq._successor) {
                    if (nextSq._hasFixedNum) {
                        allUnfixedSuccessors = false;
                        break;
                    }
                }
            ...
            if (allUnfixedSuccessors) {
                Sq nextSq = next._successor; next._sequenceNum = 0;
                if (nextSq != null) {
                    int newGroup = newGroup();
                    next._group = newGroup;
                    for (; nextSq != null; nextSq = nextSq._successor) {

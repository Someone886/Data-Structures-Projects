...
class PuzzleGenerator implements PuzzleSource {
    ...
    static Sq findUniqueSuccessor(Model model, Sq start) {
        Sq uniqueSuccessor = null;
        Sq[] unnemberedPotential = new Sq[model.width() * model.height()];
        int currentNum = 0;

        if (start.successor() != null) {
            return start.successor();
        } else if (start.sequenceNum() != 0) {
            for (Place potential: start.successors()) {
                if (start.sequenceNum() + 1
                        == model.get(potential).sequenceNum()) {
                    uniqueSuccessor = model.get(potential);
                    return uniqueSuccessor;
                }
                if (model.get(potential).sequenceNum() == 0) {
                    unnemberedPotential[currentNum] = model.get(potential);
                    currentNum += 1;
                }
            }
            if (currentNum == 1) {
                uniqueSuccessor = unnemberedPotential[0];
                return uniqueSuccessor;
            }
        } else if (start.successor() == null && start.direction() != 0) {
            for (Place potential: start.successors()) {
                if (start.connectable(model.get(potential))) {
                    unnemberedPotential[currentNum] = model.get(potential);
                    currentNum += 1;
                }
            }
    ...
    static Sq findUniquePredecessor(Model model, Sq end) {
        Sq uniquePredecessor = null;
        Sq[] unnemberedPotential = new Sq[model.width() * model.height()];
        int currentNum = 0;

        if (end.predecessor() != null) {
            return end.predecessor();
        } else if (end.sequenceNum() == 0) {
            for (Place potential: end.predecessors()) {
                if (model.get(potential).connectable(end)) {
                    unnemberedPotential[currentNum] = model.get(potential);
                    currentNum += 1;
                }
            }
            if (currentNum == 1) {
                uniquePredecessor = unnemberedPotential[0];
                return uniquePredecessor;
            }
        } else if (end.predecessor() == null && end.sequenceNum() != 1) {
            for (Place potential : end.predecessors()) {
                if (model.get(potential).sequenceNum() + 1
                        == end.sequenceNum()) {
                    uniquePredecessor = model.get(potential);
                    return uniquePredecessor;
                }
                if (model.get(potential).connectable(end)) {
                    unnemberedPotential[currentNum] = model.get(potential);

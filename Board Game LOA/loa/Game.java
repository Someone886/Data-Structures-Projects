...
class Game {
    ...
    /** Returns the player placing the next piece. */
    Player player() {
        return _board.turn().abbrev().equals(BP.abbrev())
                ? _black : _white;
    }
    ...
    /** Change _playing to a new boolean value.
     * @param playing the new boolean value for _playing*/
    void changePlaying(boolean playing) {
        _playing = playing;
    }
    ...
    /** Undo two moves. */
    private void undo() {
        if (_playing && _board.winner() == null) {
    ...
    /** A map that matches piece and its player. */
